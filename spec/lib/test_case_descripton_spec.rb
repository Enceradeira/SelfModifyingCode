...
describe TestCaseDescription do
  ...
  describe 'parse' do
    ...
    it { expect(lambda { desc.parse(',->') }).to raise_error(StandardError) }
    it { expect(lambda { desc.parse(',3->') }).to raise_error(StandardError) }
    it { expect(lambda { desc.parse('2,->') }).to raise_error(StandardError) }
    it { expect(lambda { desc.parse('->,') }).to raise_error(StandardError) }
    it { expect(lambda { desc.parse('->,3') }).to raise_error(StandardError) }
    it { expect(lambda { desc.parse('->2,') }).to raise_error(StandardError) }
  end
